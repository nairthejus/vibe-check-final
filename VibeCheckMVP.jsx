...
export default function VibeCheckMVP(){
  ...
  const [played, setPlayed] = useState([]);     // track ids
  ...
  const target = useMemo(()=>{
    let v = baseTarget.valence;
    let e = baseTarget.energy;
  ...
  // exploration increases with skips, decreases with likes
  ...
  const queue = useMemo(()=>{
    ...
    const scored = candidates.map(t => ({
      t,
      s: scoreTrack(t, target, activity, search, exploration) +
         (likes.includes(t.id) ? 0.15 : 0) -
         (dislikes.includes(t.id) ? 0.2 : 0)
    }));
    scored.sort((a,b) => b.s - a.s);
    return scored.map(x => x.t).slice(0, 9);
  ...
  function onPlay(){
    ...
    setPlayed(p => [...p, nowPlaying.id]);
  }
  function onSkip(){
    if (!nowPlaying) return;
    setDislikes(d => d.includes(nowPlaying.id) ? d : [...d, nowPlaying.id]);
    setPlayed(p => [...p, nowPlaying.id]);
  }
  function onLike(){
    if (!nowPlaying) return;
    setLikes(l => l.includes(nowPlaying.id) ? l : [...l, nowPlaying.id]);
  }
  function resetSession(){
    setPlayed([]); setLikes([]); setDislikes([]); setShowActivityNudge(false);
  }

  const metrics = {
    plays: played.length,
    likes: likes.length,
    skips: dislikes.length,
    exploration: exploration.toFixed(2),
  };
      ...
      <div className="max-w-5xl w-full">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-semibold">VibeCheck — Guest Demo</h1>
          </div>
          <button onClick={resetSession} className="px-4 py-2 rounded-2xl bg-neutral-800 hover:bg-neutral-700 text-sm">
            Reset
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-neutral-900 rounded-2xl p-4 shadow">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm text-neutral-300">Activity</p>
            </div>
          ...
          <div className="md:col-span-2 bg-neutral-900 rounded-2xl p-4 shadow">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-medium">Anchor track</h2>

                {/* Info button */}
                <button
                  onClick={()=>setShowHelp(v=>!v)}
                  className="ml-1 p-1 rounded-full hover:bg-neutral-800"
            ...
            {showHelp && (
              <div className="relative">
                <div className="absolute z-20 mt-2 w-[320px] rounded-xl border border-neutral-800 bg-neutral-900 p-3 text-sm text-neutral-300 shadow-xl">
                  <div className="font-medium mb-1">What’s happening</div>
                  <ul className="list-disc pl-5 space-y-1">
                    <li><span className="font-medium">Slider</span> sets a target valence (sad→happy) & energy (calm→intense).</li>
                    <li><span className="font-medium">Mood & Activity</span> nudge that target slightly.</li>
                    <li><span className="font-medium">Like</span> tightens the queue (exploration ↓). <span className="font-medium">Skip</span> widens it (exploration ↑).</li>
                    <li><span className="font-medium">Search</span> boosts matching title/artist a bit.</li>
                  </ul>
                  <div className="mt-2 text-xs text-neutral-500">Seeded catalog · no account required.</div>
                </div>
              </div>
            )}
                ...
                >
                  <div>
                    ...
                    <div className="text-sm text-neutral-400">
                      {nowPlaying.artist} · {nowPlaying.genre} · v{nowPlaying.valence.toFixed(2)} e{nowPlaying.energy.toFixed(2)}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={onLike} className="p-2 rounded-full hover:bg-neutral-700" title="Like">
                      <ThumbsUp className="w-5 h-5"/>
                    </button>
                    <button onClick={onSkip} className="p-2 rounded-full hover:bg-neutral-700" title="Skip">
                      <SkipForward className="w-5 h-5"/>
                    </button>
                    <button onClick={onPlay} className="p-2 rounded-full bg-neutral-100 text-neutral-900" title="Play">
            ...
            {queue.slice(1).map(t => (
              <div key={t.id} className="bg-neutral-800 rounded-xl px-4 py-3 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-neutral-400">{t.artist} · {t.genre}</div>
                </div>
            ...
            >
              ...
              >
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={()=>setShowActivityNudge(false)} className="px-3 py-2 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-sm">
                    Keep going
                  </button>
                  <button onClick={()=>setShowActivityNudge(false)} className="px-3 py-2 rounded-lg bg-neutral-100 text-neutral-900 text-sm">
                    Apply
                  </button>
                </div>
              </motion.div>
